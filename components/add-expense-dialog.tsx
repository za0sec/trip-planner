...
export function AddExpenseDialog({ open, onOpenChange, tripId, tripCurrency, onExpenseAdded }: AddExpenseDialogProps) {
  ...
  const fetchTripMembers = async () => {
    try {
      ...
      // Obtener miembros del viaje (incluyendo TODOS los estados para debug)
      ...
      if (allMembersError) {
        console.error("❌ Error fetching all trip members:", allMembersError)
      } else {
        console.log("📋 ALL trip members (all statuses):", allMembersData)
      }

      // Obtener solo miembros aceptados
      ...
      if (membersError) {
        console.error("❌ Error fetching accepted trip members:", membersError)
        return
      }
